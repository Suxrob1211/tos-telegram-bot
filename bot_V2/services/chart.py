...
class ChartDownloader:
    ...
    def _open_page(self, ticker):
        ...
        # Reklama/tooltip/banner kabi keraksiz elementlarni olib tashlaymiz
        try:
            page.evaluate("""
                () => {
                    const selectors = [
    ...
    def _capture_via_share_download(self, page):
        ...
        # Aniq data-testid asosida (finviz'ning chart toolbar'i shu konventsiyani ishlatadi),
        # fallback sifatida matn asosidagi eski selectorlar
        share_btn = page.locator(
            '[data-testid="chart-toolbar-publish"], '
            'button:has-text("Share"), a:has-text("Share"), [class*="share"]:has-text("Share")'
        ).first

        # Chart widget sekin render bo'lishi mumkin - timeout oshirildi
